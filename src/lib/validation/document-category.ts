...
/**
 * `ja`・`en`以外の任意言語1件分の名称の検証スキーマ。`documentTranslationSchema`
 * （`validation/document.ts`）と同型だが、`title`/`description`の代わりに`name`のみを持つ。
 */
...
/** 追加言語1件分（`en`を除く）の上限件数。`documentFormSchema`と同一の上限値。 */
const DOCUMENT_CATEGORY_ADDITIONAL_TRANSLATIONS_MAX_COUNT = 20;

/**
 * カテゴリの追加・編集フォームの入力値を検証する zod スキーマ。`documentFormSchema`
 * （多言語対応部分）のロジックをそのまま写経する: `ja`名称は`name`、`en`名称は`nameEn`
 * （実質必須）、追加言語は`translations`（重複不可・上限20件）。`transform`で`nameEn`を
 * `translations`の`en`行へ合成する。再パース時（Server Actions側の多重防御）も冪等。
 */
export const documentCategoryFormSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      ...
      DOCUMENT_CATEGORY_ADDITIONAL_TRANSLATIONS_MAX_COUNT
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["translations"],
        message: `translations must contain at most ${DOCUMENT_CATEGORY_ADDITIONAL_TRANSLATIONS_MAX_COUNT} additional languages`,
      });
    }

    // `nameEn`が未指定（2回目のパース、`en`は既に`translations`側へ合成されている想定）の
    // 場合のみ、`translations`内の`en`行を重複扱いしない（`documentFormSchema`と同型）。
