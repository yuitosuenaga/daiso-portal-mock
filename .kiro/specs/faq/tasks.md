...
- [x] 8. `Faq` モデルに `updatedAt` を追加する（要件9.1）
...
- [x] 9. 型・サービス層にタイムスタンプを露出する（要件9.2）
...
- [x] 10. FAQユーティリティ（新着判定・キーワード絞り込み）を実装する（要件9.3, 9.5, 10.2）
...
- [x] 11. `FaqAccordion` に更新日・新着バッジを表示する（要件9.2, 9.3, 9.4）
...
- [x] 12. 申請者側キーワード検索（`FaqListClient`）を実装する（要件10）
...
- [x] 14. `tsc --noEmit`・`npm run lint`・`npm test`・`npm run build` が全て通ることを確認する
  - _Requirements: 8.1〜10.6_
  - _Depends: 7, 11, 12, 13_

- [x]* 15. 多言語・レスポンシブ・改行表示のE2E確認を行う
...
- [x] 22. FAQの質問・回答を多言語（言語別データ・選択ロケール表示）対応にする（要件12）

- [x] 22.1 Prismaに`FaqTranslation`モデルを追加しマイグレーションを生成する（要件12.1, 12.2, 12.9）
...
- [x] 22.2 `Faq`型に`translations`を追加する（要件12.3）
...
- [x] 22.3 `faq-mapper.ts`を新規作成し表示解決関数を実装する（要件12.4, 12.8）
...
- [x] 22.4 申請者側読み取りに`locale`解決を通す（要件12.5, 12.8）
...
- [x] 22.5 申請者側一覧を選択ロケールの質問・回答で表示する（要件12.6, 12.7, 12.10）
