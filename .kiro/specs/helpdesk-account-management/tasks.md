...
- [ ] 1. `ApplicantUser`モデルに`isActive`フィールドを追加し、マイグレーションを作成する
...
- [ ] 6. (P) ApplicantUserフォームのzodスキーマを実装する
...
- [ ] 8. `company-service.ts`に管理画面向けCRUD関数を追加する
...
- [ ] 9. `applicant-user-service.ts`を新規実装する
...
- [ ] 10. 無効化されたアカウントのログイン拒否を`authorize.ts`に追加する
...
- [ ] 11. CompanyのServer Actionsを実装する
...
- [ ] 12. ApplicantUserのServer Actionsを実装する
...
- [ ] 13. `CompanyManagementList`・`CompanyManagementListClient`を実装する
...
- [ ] 18. (P) `ToggleApplicantUserActiveButton`を実装する
...
- [ ] 20. ヘルプデスク側ApplicantUser管理ページを実装する
...
- [ ] 21. `HelpdeskSidebar`に販社管理のナビゲーション項目を追加する
...
- [ ] 23. 無効化されたアカウントのログイン拒否を確認する
  - ApplicantUserを無効化した後、当該アカウントの（無効化前の）正しいメールアドレス・パスワードでのログインが失敗することを確認する
  - 再有効化後、同じ認証情報でログインが成功することを確認する
  - _Requirements: 7.5, 7.7_
  - _Depends: 21_

- [ ] 24. `tsc --noEmit`・`npm run lint`・`npm test`・`npm run build`が全て通ることを確認する
  - _Requirements: 1.1〜11.1_
  - _Depends: 22, 23_

- [ ]* 25. 多言語表示・レスポンシブ表示を確認する
