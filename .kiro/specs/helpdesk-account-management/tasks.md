...
- [ ] 26. `createCompany`で`AnnouncementRecipient`を同期生成する
...
- [ ] 27. 作成直後の`Company`がお知らせトラッキング対象に含まれることを結合テストで確認する
...
- [ ] 28. 既存`Company`補完のbackfillスクリプトを追加する
...
- [ ] 29. `ApplicantUser`作成フローが`AnnouncementRecipient`に触れないことを確認する
