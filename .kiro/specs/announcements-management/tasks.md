...
- [x] 21.3 会社単位の自己申告状態を読み取る関数を実装する
...
- [x] 22. コア: 申請者セッションでラップするAPI層関数の実装
- [x] 22.1 会社単位の確認済み・実施済み記録をセッション経由で受け付けるAPI関数を実装する
...
- [x] 22.2 自社の自己申告状態を取得するAPI関数を実装する
  - 申請者セッションから会社コードを取得し、会社単位の読み取り関数の結果を返す
  - _Requirements: 23.5_
  - _Boundary: lib/api/announcement-tracking.ts_
  - _Depends: 21.3_

- [x] 23. 統合: Server Actionsの実装と関連ルートの再検証
- [x] 23.1 確認済み・対応完了を記録するServer Actionsを実装する
...
- [x] 24.2 (P) API層の可視性ガードの単体テストを実装する
  - 下書き・配信対象外・公開期間外のお知らせに対して記録が行われないことを検証する
  - 全テストがパスすることで完了とする
  - _Requirements: 23.3, 23.4_
  - _Depends: 22.1_

- [x] 24.3 統合テスト: 記録結果が既存の集計・未対応者一覧に反映されることを確認する
...
- [x] 27.2 作成・更新時に添付・紐づけを永続化する
  ...
  - 作成・更新後に添付・紐づけが正しく永続化され、更新のたびに古い添付・紐づけが除去されることで完了とする
  - _Requirements: 24.7_
...
- [x] 30.1 `AnnouncementDetail`に「添付ファイル」セクションを追加する
  ...
  - 一覧画面・ダッシュボードウィジェットのレイアウト・データ取得は変更しない
  - 対象国・対象会社の閲覧者には紐づけドキュメントが表示され、対象外の閲覧者には表示されないことをブラウザで確認できることで完了とする
  - _Requirements: 24.8, 24.9, 24.10, 24.11_
...
- [ ] 32.2 日本語・英語両ロケール、タブレット幅（768px）での表示確認
  - 添付・紐づけUIおよび詳細画面の添付ファイルセクションが、両ロケール・タブレット幅で横スクロールを起こさず表示されることをブラウザで確認する
  - 上記確認が問題ないことで完了とする
  - _Requirements: 24.1, 24.4, 24.8_
  - _Depends: 30.1_

- [x] 33. Core: 管理フォーム（AnnouncementForm）へのPDFインラインプレビュー統合
- [x] 33.1 直接アップロードのPDF添付にインラインプレビューを追加する
...
- [x] 33.2 紐づけドキュメントにインラインプレビューを追加する
...
- [x] 34. (P) Core: 詳細画面（AnnouncementDetail）へのPDFインラインプレビュー統合
- [x] 34.1 (P) 直接アップロード添付をMIMEタイプで振り分けてプレビュー表示する
...
- [x] 35. 検証: コンポーネントテスト
- [x] 35.1 (P) `AnnouncementForm`のPDFプレビュー表示の単体テストを実装する
  - PDF添付・紐づけドキュメントを渡したときに件数分のプレビュー要素が描画され、画像添付では描画されないことを検証する
  - 全テストがパスすることで完了とする
  - _Requirements: 25.1, 25.2, 25.3_
  - _Depends: 33.2_

- [x] 35.2 (P) `AnnouncementDetail`のPDFプレビュー表示の単体テストを実装する
...
- [x] 37. データ基盤: スキーマ拡張とマイグレーション
- [x] 37.1 `AnnouncementTranslation`・`AnnouncementNotificationLog`モデル、`ApplicantUser.preferredLocale`を追加する
...
- [x] 38. コア: メール送信基盤（mailer）の実装
- [x] 38.1 `nodemailer`ラッパー（`mailer.ts`）を実装する
...
- [x] 39. コア: 言語解決・宛先解決・通知オーケストレーションの実装
- [x] 39.1 `resolveAnnouncementContent`・`targetApplicantUsersWhere`を実装する
...
- [x] 39.2 `announcement-notifications.ts`（`notifyAnnouncementPublished`/`notifyAnnouncementReminder`）を実装する
...
- [x] 40. 統合: 公開・リマインド操作からの通知呼び出し結線
- [x] 40.1 `createAnnouncementRecord`/`updateAnnouncementRecord`から公開時に`notifyAnnouncementPublished`を呼び出す
...
- [x] 40.2 `sendAnnouncementReminders`から`notifyAnnouncementReminder`を呼び出す
...
- [x] 41. コア: フォームの言語別入力
- [x] 41.1 `announcementFormSchema`に`titleEn`/`bodyEn`/`translations`を追加する
...
- [x] 41.2 `AnnouncementForm`に言語タブUIを追加する
...
- [x] 42. 統合: 保存経路への翻訳データ反映
- [x] 42.1 `createAnnouncementRecord`/`updateAnnouncementRecord`が`translations`を永続化する
...
- [x] 43. 検証: 単体テスト
- [x] 43.1 (P) `mailer`・`announcement-notifications`の単体テストを実装する
  - SMTP未設定時に`skipped`、送信失敗時に`failed`として記録されることを検証する
  - 全テストがパスすることで完了とする
  - _Requirements: 26.1, 29.1, 29.2_
  - _Depends: 39.2_

- [x] 43.2 (P) `resolveAnnouncementContent`・`targetApplicantUsersWhere`の単体テストを実装する
  - `ja`固定、`en`・追加言語の解決、未登録言語での`ja`フォールバックを検証する
  - 全テストがパスすることで完了とする
  - _Requirements: 31.4, 33.2_
  - _Depends: 39.1_

- [x] 43.3 (P) `announcementFormSchema`の単体テストを実装する
...
- [ ] 44. 検証: 統合テスト・実機確認
- [x] 44.1 公開・リマインド時の通知送信の統合テストを実装する
...
- [x] 48. コア: 配信対象拡大の差分検出と追加通知
- [x] 48.1 `AnnouncementNotificationKind`に`target_added`を追加しマイグレーションを作成する
...
- [x] 48.2 `addedTargetApplicantUsersWhere`を`announcement-mapper.ts`に実装する
...
- [x] 48.3 `notifyAnnouncementTargetExpanded`を`announcement-notifications.ts`に実装する
...
- [x] 48.4 `updateAnnouncementRecord`に差分検出と追加通知の呼び出しを結線する
...
- [x] 49. 検証: 単体・統合テスト
- [x] 49.1 (P) `addedTargetApplicantUsersWhere`の単体テストを実装する
...
- [x] 49.2 (P) `notifyAnnouncementTargetExpanded`の単体テストを実装する
...
- [x] 49.3 `updateAnnouncementRecord`の統合テストを実装する
...
- [x] 50. コア: フォールバック順序を英語優先に変更する
- [x] 50.1 `resolveAnnouncementContent`に`en`優先フォールバックを追加する
...
- [x] 50.2 通知メール詳細リンクのUIロケールフォールバックを`en`に変更する
...
- [x] 51. 検証: 既存テストの期待値更新と追加
- [x] 51.1 (P) `resolveAnnouncementContent`のテストを`en`優先に更新する
  ...
  - _Depends: 50.1_
- [x] 51.2 (P) 通知メールのフォールバックテストを`en`優先に更新する
  ...
  - _Depends: 50.1, 50.2_
- [x] 51.3 統合テスト: en翻訳のみのお知らせのth宛通知がen本文・enリンクになることを確認する
  - `en`翻訳のみを登録したお知らせで、`preferredLocale: "th"`の`ApplicantUser`向け公開通知が`en`本文・`/en/`詳細リンクで送信されることを検証する
  - _Requirements: 36.4, 36.6_
  - _Depends: 50.1, 50.2_

- [x] 52. お知らせ削除確認をアプリ内モーダル（ConfirmDialog）へ置き換え、対象タイトルを明示する（2026-07-22 追記 / 要件37）
  - `DeleteAnnouncementButton.tsx`の`window.confirm()`を廃止し、共通`ConfirmDialog`（`src/components/ui/confirm-dialog.tsx`, helpdesk-portal-layout要件18）でラップする。確認押下時のみ`deleteAnnouncementAction`実行→`/helpdesk/announcements`へ遷移（既存ロジックを`onConfirm`へ移設）、`isPending`を伝播する
...
- [x]* 52.1 `DeleteAnnouncementButton.test.tsx` をConfirmDialogベースへ更新する
...
- [x] 55. コア: トリガー非依存の中核関数 `runAnnouncementAutoEscalation` を新規実装する
  ...
  - 当日（`now` のローカル暦日開始以降）に `kind IN ('escalation','reminder')` の `AnnouncementNotificationLog` を持つ `recipientEmail` 集合を取得し、除外集合として `notifyAnnouncementEscalation` に渡す（要件38.5, 38.7）
  - 送信対象が生じた未対応担当者の `AnnouncementRecipientStatus.reminderSentAt = now` を `upsert`（既存 `sendAnnouncementReminders` と同一パターン。要件38.6）
  - 全体を `try/catch` で包み `throw` しない。件数要約を返す（要件38.8）
...
- [x]* 59.1 (P) `announcement-escalation.test.ts` を実装する（`now` 固定注入、`getAnnouncementRecipientStatuses`/`notifyAnnouncementEscalation`/`prisma` をモック）
  ...
  - 連続2回呼び出しで2回目は送信が増えない（冪等・38.8）、内部例外でも `throw` しない（38.8）
  - _Requirements: 38.1, 38.2, 38.5, 38.6, 38.7, 38.8_
  - _Depends: 55_
...
- [x] 61. Prisma スキーマに `AnnouncementReadReceipt` を追加しマイグレーションAを作成する
...
- [x] 62. 型定義（`src/types/announcement-recipient.ts`）を個人単位/会社単位に整理する
...
- [x] 63. サービス層に確認済みの個人単位ロジックを実装する（`announcement-service.ts`）
...
- [x] 64. 個人単位の既読リマインド送信を実装する（`announcement-service.ts` + `announcement-notifications.ts`）
...
- [x] 65. API 層（`lib/api/announcement-tracking.ts`）にヘルプデスク向け関数を追加する
...
- [x] 66. ヘルプデスクUIを個人単位（確認済み）＋会社単位（実施済み）に更新する
...
- [x] 68. バックフィルスクリプトを実装し実行する（confirmedAt → 個人受信レシート）
...
- [x] 69. `AnnouncementRecipientStatus.confirmedAt` を削除するマイグレーションB（バックフィル後）
